...
async function sendToFacebookCAPI(leadData: {
  name: string;
  phone: string;
  zipCode: string;
  state: string | null;
  ...
  if (leadData.zipCode) userData.zp = hashData(leadData.zipCode);
  if (leadData.state) userData.st = hashData(leadData.state);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { name, phone, zipCode, ageRange, submittedAt, userAgent, pageUrl } = body;

    // Validate required fields
    if (!name || !phone || !zipCode || !ageRange) {
    ...
    // Get Facebook cookies from request (if available)
    ...
    // Send to Facebook CAPI and Webhook in parallel
    const [fbResult, webhookResult] = await Promise.all([
      sendToFacebookCAPI({
        name,
        phone,
        zipCode,
